...
ctable <- coef(summary(og_ordered_logit_control))
p <- pnorm(abs(ctable[, "t value"]), lower.tail = FALSE) * 2
(ctable <- cbind(ctable, "p value" = p))
ci <- confint(og_ordered_logit_control)
exp(cbind(OR = coef(og_ordered_logit_control), ci))
...
ctable <- coef(summary(og_ordered_logit_recip))
p <- pnorm(abs(ctable[, "t value"]), lower.tail = FALSE) * 2
(ctable <- cbind(ctable, "p value" = p))
ci <- confint(og_ordered_logit_recip)
exp(cbind(OR = coef(og_ordered_logit_recip), ci))

#stargazer(og_ordered_logit_control, )
```
...
ctable <- coef(summary(new_ordered_logit))
p <- pnorm(abs(ctable[, "t value"]), lower.tail = FALSE) * 2
(ctable <- cbind(ctable, "p value" = p))
ci <- confint(new_ordered_logit)
exp(cbind(OR = coef(new_ordered_logit), ci))
